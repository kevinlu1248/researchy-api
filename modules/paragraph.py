...
class Paragraph:
    ...
    __IMPORTANCE_RATIO = .3

    def __init__(self, html, vocab=None):
        self.html = html
    ...
    def __compare_priority(a, b):
        # add length priority
        return Paragraph.__priority.get(a['tag'], 0) > Paragraph.__priority.get(b['tag'], 0)
    ...
    def __init_html_tags(self):
        ...
        for match in Paragraph.html_tag_pattern.finditer(self.__spaced_raw_text):
            ...
            self.__html_tags.append({
                "start": start,
                "end": end,
                "tag": tag
            })
            self.__spaced_raw_text = "".join(
                (self.__spaced_raw_text[:start], " " * (end - start), self.__spaced_raw_text[end:]))

    def fivefilters_get_terms(self, max=None):
        if max is None:
            max = int(len(set(self.spaced_raw_text))
                      * Paragraph.__IMPORTANCE_RATIO)
        # print(max)
        data = {
            "text": self.raw_text,
            "output": "json",
    ...
    def entities(self):
        if not hasattr(self, "_Paragraph__entities"):
            doc = Paragraph.nlp(self.spaced_raw_text)
            self.__entities = [{
                "tag": ent.label_,
    ...
    def annotated_with_entities(self):
        if not hasattr(self, "_Paragraph__annotated_with_entities"):
            ...
            key_terms, html_tags, ents = deepcopy(self.key_term_indices), deepcopy(
                self.html_tags), deepcopy(self.entities)
            # print(key_terms)
            all_tags = sorted(key_terms + ents,
    ...
    #         print(Website.get_entities_from_text(new_s)[::-1])
            for ent in all_tags[::-1]:
                self.__annotated_with_entities = Paragraph.__insert_helper(
                    self.__annotated_with_entities, "</{}>".format(ent['tag'].lower()), ent['end'])
                self.__annotated_with_entities = Paragraph.__insert_helper(
                    self.__annotated_with_entities, "<{}>".format(
                        ent['tag'].lower()), ent['start'])
